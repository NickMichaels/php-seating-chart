...
class SeatingChart {
     ...
     * creates the data set and defines the initial seats available
     *
     * @param int $rows
     * @param int $columns
    ...
    public function __construct($rows, $columns, $bestSeat) {
        ...
        if ( ($bestSeatInd['bestRow'] > $rows) || ($bestSeatInd['bestCol'] > $columns) ) {
    ...
    public static function parseBestSeat($bestSeat) {
        ...
        // This assumes the that format is RXCY, if that were to flip, the indexes 
        // would be wrong
    ...
    public function handleInitialReservations($initRes) {
        ...
        foreach ($initRes as $index => $res) {
            // Lets just go ahead and reuse parseBestSeat here
            $seatInd = self::parseBestSeat($res);
     ...
     * reserves the seats and outputs an array of strings denoting the reservations made
     *
     * @param array $newRes
     * @return array 
     */
